...
export default function Page() {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        const itemsToUpdate = cartData.items.filter(
          (item) => item.quantity > item.variant.stock_level
        );
  ...
  const updateItemQuantity = useCallback(
    async (itemId, newQuantity) => {
      ...
      try {
        await updateProduct(itemId, newQuantity);
        const updatedCart = await getCart();
        setCart(updatedCart);
      } catch (error) {
        console.error('Errore nell\'aggiornare la quantità dell\'articolo:', error);
      } finally {
        setLoadingItems((prev) => prev.filter((id) => id !== itemId));
      }
    },
    [updateProduct, getCart]
  );
  ...
  return (
    <div className="w-full h-screen flex flex-col p-8 px-11">
      <h1 className="uppercase text-[4.375rem] leading-none font-sans font-light text-black text-center">
        Revisione finale
      </h1>
        ...
        {cart.items.map((item) => (
          <div key={item.id} className="flex items-start justify-between py-4">
            ...
            <div className="ml-auto flex flex-col justify-between">
              ...
              <CounterComponent
                stockLevel={item.variant.stock_level}
                quantity={Math.min(item.quantity, item.variant.stock_level)}
      ...
      <p className="mt-8 uppercase text-[2.06125rem] leading-none font-sans font-medium text-black text-center">
        Totale da pagare per {cart.item_quantity} articoli: €
